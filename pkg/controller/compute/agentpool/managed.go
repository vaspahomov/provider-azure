...
// Error strings.
const (
	errNotAgentPool    = "managed resource is not a AgentPool"
	errCreateAgentPool = "cannot create AgentPool"
	errGetAgentPool    = "cannot get AgentPool"
	errDeleteAgentPool = "cannot delete AgentPool"
)

// SetupAgentPool adds a controller that reconciles AKSClusters.
func SetupAgentPool(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter) error {
...
type external struct {
	kube          client.Client
	client        containerserviceapi.AgentPoolsClientAPI
...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	c, err := e.client.Get(ctx, cr.Spec.AgentPoolParameters.ResourceGroupName, cr.Spec.AgentPoolParameters.AKSClusterName, meta.GetExternalName(cr))
	...
	upToDate := !compute.AgentPoolNeedUpdate(cr, &c)
...
func (e *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	_, err := e.client.CreateOrUpdate(ctx, cr.Spec.ResourceGroupName, cr.Spec.AKSClusterName, meta.GetExternalName(cr), compute.NewAgentPool(cr))
	if err != nil {
...
func (e *external) Delete(ctx context.Context, mg resource.Managed) error {
	...
	_, err := e.client.Delete(ctx, cr.Spec.AgentPoolParameters.ResourceGroupName, cr.Spec.AgentPoolParameters.AKSClusterName, meta.GetExternalName(cr))
	if azure.IsNotFound(err) {
		return nil
	}
	if err != nil {
