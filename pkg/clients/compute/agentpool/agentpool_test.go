...
func TestNeedUpdate(t *testing.T) {
	tests := []struct {
		...
		name: "NodeCount changed, but MinNodeCount and MaxNodeCount not changed",
		c: agentPoolFromParams(v1alpha3.AgentPoolParameters{
			NodeCount:    azure.ToInt32Ptr(1),
...
func TestNewProfile(t *testing.T) {
	tests := []struct {
		...
		want: containerservice.ManagedClusterAgentPoolProfile{
			Name:         azure.ToStringPtr(consts.AgentPoolProfileName),
...
func TestUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if !reflect.DeepEqual(tt.c, tt.want) {
			t.Errorf("After Update() AgentPool = %v, want %v", tt.c, tt.want)
		}
	}
}
